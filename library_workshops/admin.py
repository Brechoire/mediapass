...
class WorkshopAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informations générales", {"fields": ("title", "description", "location")}),
        (
            "Dates et horaires",
        ...
        ("Communication", {"fields": ("newsletter",)}),
        ("Visuel", {"fields": ("poster",)}),
        (
            "Métadonnées",
            {
    ...
    def capacity_status(self, obj):
        ...
        if obj.is_full:
            ...
            return format_html(
                '<span class="badge bg-warning">{} places</span>', obj.available_spots
            )
        else:
            return format_html(
                '<span class="badge bg-success">{} places</span>', obj.available_spots
            )
...
class WorkshopParticipantAdmin(admin.ModelAdmin):
    ...
    list_filter = ["status", "registration_date", "workshop__start_date", "added_by"]
    search_fields = ["first_name", "last_name", "email", "workshop__title"]
    readonly_fields = ["registration_date"]
    date_hierarchy = "registration_date"

    fieldsets = (
        ("Informations personnelles", {"fields": ("first_name", "last_name", "age")}),
        ("Contact", {"fields": ("email", "phone")}),
        ("Inscription", {"fields": ("workshop", "status", "registration_date")}),
        ("Gestion", {"fields": ("notes", "added_by"), "classes": ("collapse",)}),
    )
...
class RecurrencePatternAdmin(admin.ModelAdmin):
    ...
    workshop_count.short_description = "Ateliers générés"


@admin.register(SchoolHoliday)
class SchoolHolidayAdmin(admin.ModelAdmin):
    list_display = ["name", "start_date", "end_date", "zone"]
    list_filter = ["zone", "start_date"]
    search_fields = ["name"]

...
class WeatherService:
    """Service de récupération des données météo via Open-Meteo (gratuit, sans clé)."""
    ...
    WMO_LABELS = {
        ...
        95: "Orage", 96: "Orage + grêle", 99: "Orage fort + grêle",
    }

    @staticmethod
    def fetch_range(start_date, end_date):
        """Récupère les données météo Open-Meteo et les stocke en base."""
        import requests
        ...
        try:
            resp = requests.get(WeatherService.BASE_URL, params=params, timeout=15)
            resp.raise_for_status()
            data = resp.json()
        except (requests.RequestException, ValueError) as e:
            import logging
            logging.getLogger(__name__).warning("Erreur météo : %s", e)
            return
        ...
        for i, date_str in enumerate(dates):
            try:
                d = __import__("datetime").datetime.strptime(date_str, "%Y-%m-%d").date()
            ...
            WeatherData.objects.update_or_create(
                ...
                defaults={
                    ...
                    "precipitation": precips[i] if i < len(precips) and precips[i] is not None else 0,
                    "weather_code": codes[i] if i < len(codes) else None,
                },
            )

    @staticmethod
    def get_or_fetch(start_date, end_date):
        """Récupère depuis le cache ou fetch les dates manquantes."""
        from .models import WeatherData
        ...
        if missing:
            WeatherService.fetch_range(min(missing), max(missing))
